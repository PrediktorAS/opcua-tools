...
def supertypes_of_nodes(types_to_supertype: List[str], type_nodes: pd.DataFrame,
...
def hierarchical_references_trg_has_modelling_rule(references:pd.DataFrame,
    ...
    hierarchical_refs_has_modelling_rule = hierarchical_refs_has_modelling_rule.reset_index().rename(
        columns={'index': 'Trg'})
...
def has_subtype_references(references: pd.DataFrame, type_nodes: pd.DataFrame, id_col: str) -> pd.DataFrame:
    hst_id = type_nodes.loc[
        (type_nodes['NodeClass'] == 'UAReferenceType') & (type_nodes['BrowseName'] == 'HasSubtype'), id_col].iloc[0]
    subtype = references[references['ReferenceType'] == hst_id].copy()
    return subtype


def has_type_definition_references(references: pd.DataFrame, type_nodes: pd.DataFrame) -> pd.DataFrame:
    htd_id = type_nodes.loc[(type_nodes['NodeClass'] == 'UAReferenceType') &
                            (type_nodes['BrowseName'] == 'HasTypeDefinition'), 'id'].iloc[0]
    htd = references[references['ReferenceType'] == htd_id].copy()
    return htd


def has_property_references(inst_references: pd.DataFrame, type_references: pd.DataFrame,
                            type_nodes: pd.DataFrame) -> pd.DataFrame:
    propref_id = type_nodes.loc[(type_nodes['NodeClass'] == 'UAReferenceType') &
                                (type_nodes['BrowseName'] == 'HasProperty'), 'id'].iloc[0]
    prop_ref = constrain_to_reference_type(inst_references, type_nodes, type_references, [propref_id])
    return prop_ref


def has_modelling_rule_references(inst_references: pd.DataFrame, type_references: pd.DataFrame,
                                  type_nodes: pd.DataFrame) -> pd.DataFrame:
    propref_id = type_nodes.loc[(type_nodes['NodeClass'] == 'UAReferenceType') &
                                (type_nodes['BrowseName'] == 'HasModellingRule'), 'id'].iloc[0]
    prop_ref = constrain_to_reference_type(inst_references, type_nodes, type_references, [propref_id])
    return prop_ref


def hierarchical_references(inst_references: pd.DataFrame, type_references: pd.DataFrame,
                            type_nodes: pd.DataFrame) -> pd.DataFrame:
    hierref_id = type_nodes.loc[(type_nodes['NodeClass'] == 'UAReferenceType') &
                                (type_nodes['BrowseName'] == 'HierarchicalReferences'), 'id'].iloc[0]
    prop_ref = constrain_to_reference_type(inst_references, type_nodes, type_references, [hierref_id])
    return prop_ref
...
def fast_transitive_closure(references: pd.DataFrame) -> pd.DataFrame:
    ...
    assert (references['Src'] == references['Trg']).sum() == 0, 'There should be no references r such that r(n,n)'
    ...
    sparmat = coo_matrix((data, (src_codes, trg_codes)), shape=(uniques.shape[0], uniques.shape[0]), dtype=bool)
...
def fast_hierarchy_transitive_closure(inst_references: pd.DataFrame,
                                      constrained_references: pd.DataFrame,
                                      type_nodes: pd.DataFrame,
                                      type_references: pd.DataFrame,
                                      interesting_types: pd.Series) -> pd.DataFrame:
    ...
    types = type_nodes[type_nodes['NodeClass'].str.endswith('Type')].set_index('BrowseName')
    interesting_ids_types = types.loc[pd.Index(interesting_types), 'id'].to_list()

    sub_inter_types = subtypes_of_nodes(interesting_ids_types, type_nodes, type_references)
...
def signal_variables(instance_references: pd.DataFrame, type_nodes: pd.DataFrame,
                     type_references: pd.DataFrame) -> pd.DataFrame:
    reftypes = type_nodes[type_nodes['NodeClass'] == 'UAVariableType']
    signalvar_type = reftypes.loc[reftypes['BrowseName'] == 'BaseDataVariableType', 'id'].iloc[0]
...
def property_variables(instance_references: pd.DataFrame, type_nodes: pd.DataFrame,
                       type_references: pd.DataFrame) -> pd.DataFrame:
    reftypes = type_nodes[type_nodes['NodeClass'] == 'UAVariableType']
...
def typing_transitive_reflexive(type_nodes: pd.DataFrame, type_references: pd.DataFrame) -> pd.DataFrame:
    ...
    subtype_closure = pd.concat([subtype_closure, pd.DataFrame({'Src': unique_types, 'Trg': unique_types})])
    return subtype_closure


def subtypes_of_nodes(types_to_subtype: Union[pd.Series, List[str]], type_nodes: pd.DataFrame,
                      type_references: pd.DataFrame) -> pd.DataFrame:
    # Computes the closure
    closure = typing_transitive_reflexive(type_nodes, type_references)
...
def closest_parent(inst_nodes: pd.DataFrame, inst_references: pd.DataFrame, type_nodes: pd.DataFrame,
                   type_references: pd.DataFrame,
                   interesting_types: List[str], uavar_type: str) -> pd.DataFrame:
    ...
    inst_references = inst_references[~inst_references.index.isin(inst_inter_type_index)].reset_index()
    ...
    if uavar_type == 'Property':
        inst_references = inst_references.set_index('Trg')
        inst_references = inst_references[~inst_references.index.isin(signals_index)].reset_index()

    inst_references_trans = fast_transitive_closure(inst_references[['Src', 'Trg']])
    inst_nodes_ns_index = inst_nodes[['id', 'ns']].set_index('id')
    inst_references_trans = inst_references_trans.set_index('Src').join(inst_nodes_ns_index).reset_index().rename(
        columns={'index': 'Src', 'ns': 'Src_ns'})
    inst_references_trans = inst_references_trans.set_index('Trg').join(inst_nodes_ns_index).reset_index().rename(
        columns={'index': 'Trg', 'ns': 'Trg_ns'})
    ...
    if uavar_type == 'Property':
        ...
        inst_references_trans = inst_references_trans[inst_references_trans.index.isin(pd.Index(properties))]
        inst_references_trans = inst_references_trans.reset_index().set_index('Src')
        inst_references_trans = inst_references_trans[
            (inst_references_trans.index.isin(signals_index) | inst_references_trans.index.isin(inst_inter_type_index))]
        inst_references_trans['is_signal'] = inst_references_trans.index.isin(signals_index)
        inst_references_trans = inst_references_trans.reset_index()
        inst_references_trans = inst_references_trans.sort_values(by=['Trg', 'is_signal'], ascending=[True, False])
        ...
        return inst_references_trans.rename(
            columns={'Src': 'instance', 'Trg': 'property', 'Src_ns': 'instance_ns', 'Trg_ns': 'property_ns'})

    if uavar_type == 'Signal':
        inst_references_trans = inst_references_trans.set_index('Trg')
        inst_references_trans = inst_references_trans[inst_references_trans.index.isin(signals_index)]
        inst_references_trans = inst_references_trans.reset_index().set_index('Src')
        inst_references_trans = inst_references_trans[
            inst_references_trans.index.isin(inst_inter_type_index)].reset_index()
        inst_references_trans = inst_references_trans.drop_duplicates(keep='first')
        return inst_references_trans.rename(
            columns={'Src': 'instance', 'Trg': 'variable', 'Src_ns': 'instance_ns', 'Trg_ns': 'variable_ns'})


def constrain_to_reference_type(inst_references: pd.DataFrame, type_nodes: pd.DataFrame, type_references: pd.DataFrame,
                                reference_types: List[str]) -> pd.DataFrame:
    subtypes_of_reference = subtypes_of_nodes(reference_types, type_nodes, type_references)
    inst_references = inst_references.set_index('ReferenceType')
    inst_references = inst_references[inst_references.index.isin(pd.Index(subtypes_of_reference['subtype']))]
