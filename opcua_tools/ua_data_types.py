...
class UnparsedUAExtensionObject(UABuiltIn):
    ...
    def xml_encode(self, include_xmlns: bool) -> str:
        ...
        if include_xmlns:
            x += ' ' + UAXMLNS_ATTRIB
        x += '>\n'
        x += '<TypeId>' + (self.type_nodeid.xml_encode(include_xmlns=False) if self.type_nodeid is not None else '') + '</TypeId>\n'
...
class UAEngineeringUnits(UAData):
    ...
    def xml_encode(self, include_xmlns: bool) -> str:
        ...
        body_contents += '<Locale>' + (self.display_name.locale if self.display_name.locale is not None else '') + '</Locale>\n'
        body_contents += '<Text>' + (self.display_name.text if self.display_name.text is not None else '') + '</Text>\n'
        body_contents += '</DisplayName>\n'
        body_contents += '<Description>'
        body_contents += '<Locale>' + (self.description.locale if self.description.locale is not None else '') + '</Locale>\n'
